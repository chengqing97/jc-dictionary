...
Future<LookupResult> lookup(String toSearch, http.Client client) async {
  ...
  try {
    ...
    if (isTranslate) {
      return LookupResult(keyword: toSearch, definition: grabTranslation(response));
    }

    final isChinese = RegExp(r'英语怎么说').hasMatch(response);

    if (isChinese) {
      return LookupResult(keyword: toSearch, definition: grabEnglishWords(response));
    }
...
String? grabFormalDefinition(String response) {
  ...
  final definition = definitionsRaw.map((item) => item.group(0) ?? "").join("\n");
...
Future<VoiceUrl> getVoiceUrl(String keyword, http.Client client) async {
  ...
  var ukPath = RegExp(r"media/english/uk_pron/[\S]*.mp3").firstMatch(response)?.group(0);
  var usPath = RegExp(r"media/english/us_pron/[\S]*.mp3").firstMatch(response)?.group(0);

  const host = "https://dictionary.cambridge.org/";

  return VoiceUrl(uk: ukPath == null ? null : host + ukPath, us: usPath == null ? null : host + usPath);
}
